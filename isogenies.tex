...
$\ell$-torsion. A theorem of \TT[G_\QQ]ibet~\cite[Proposition 2]{ribet:mult_p_finite}
...
\begin{proof}
    ...
    through $\Gal(K/\QQ)$. By the proof of~\cite[Lemma
...
\begin{proposition}\label{prop:things_are_equivalent}
    Suppose $I,\q$ are nonzero ideals such that $bI = a\q$ for $a,b\in \TT_A
    ^\times$ and $\phi=a\circ b^{-1}$. Then $A[I]\sim A[\q]$. Moreover, suppose
    $M'$ is a $G_\QQ$-submodule of $A(\QQbar)$, then $A[I]+M'\sim
    A[\q]+\phi(M')$.
\end{proposition}
\begin{proof}

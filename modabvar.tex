...
\newcommand{\ws}[1]{\textcolor{blue}{\textsf{ws: #1}}}
...
\title{Computing with Isogenies Between Abelian Varieties of $\GL_2$-type\ws{Title seems wrong now...}}
...
\begin{abstract}
  ...
  the modular degree, computing special values of $L$-functions,
  enumeration of the isogeny class, and
  computing Tamagawa numbers. Though we do not completely solve
  all problems listed above, none of our algorithms use defining
...
Since Khare and Wintenberger \ws{add citation} have now completely proved Serre's
...
\begin{remark}
  ...
  is also a topic for future investigation, and we expect that
  many of the algorithms we give
  here generalize to that context.
  \ws{Cite Dummigan-Stein-Watkins.}
...
\ws{It would be clearer to first say that we explain how we represent $J$, which is of course what we do below.}
...
\ws{Since we said we give $V$ by reduced row echelon form, we could similar say that we give $L$ by
its Hermite normal form basis.}
...
\ws{Obviously, we need a section with some simple examples here, which are likely to be familiar to readers.  Good simple explicit examples would be: 11a2 (showing nontrivial $L$), 37a1, 43a1 and 43b1 (dim 2).}
...
Using ["Modular Forms, A Computational Approach"], we can compute
representatives of Galois conjugacy classes of newforms of $\Gamma_1(N)$. The
isogeny classes of simple abelian subvarieties of $J$ are correspondnce with
the Galois conjugary classes of newforms of $\Gamma_1(N)$.
...
Then the sum $A + A' \subset J$ is given by $(V+V', J)$.
...
\ws{Title should be "Intersections" to be consistent with adjacent titles.}
...
$$
The component group is isomorphic to the torsion subgroup of the kernel of
...
Let $G$ be a finite subgroup of a modular abelian variety $(V,L,J)$. Since $G$
is finite, it must be of the form $\Lambda/L$ with $e \Lambda \subset L$, where
$e$ is the exponent of $G$. We will define the finite subgroup $G$ with the
tuple $(L, K, A)$, where $G=L/\Lambda \subseteq A = V/\Lambda$ and $K$ is the
field of definition.
...
Let $A=V/L$. The torsion subgroup of $A$ is $\Q L/L$ so the $n$-torsion
subgroup is $\frac{1}{n}L/L$.
...
Given a modular abelian variety, this algorithm computes the
...
\subsubsection{Point Counting over Finite Fields}

Let $p$ be an odd prime not dividing $N$. Using the theory of formal groups, we
can show reduction map $A_f(\Q)\to A_f(\FF_p)$ is injective on torsion points \ws{reference; probably in
my thesis or some paper with Amod Agashe}.
Hence, for any set $P$ of odd primes not dividing $N$,
the gcd $\GCD(\{\#A_f(\FF_p):p\in P\})$ is a multiple of the $\Q$-rational torsion order. To
compute $\#A_f(\FF_p)$, we use the result in Agashe-Stein \ws{citation}.


\subsubsection{The Rational Torsion Order}

When $A$ is not an elliptic curve, there is no known algorithm for explicitly
computing the rational torsion order. There are 39 newforms of level up to 30.
We are able to compute the rational torsion order of the corresponding abelian
variety for 13 of them using these methods.
...
\begin{enumerate}
\item{} [Hermite Normal Form] Find the Hermite Normal Form $H'$ of $M^t$. Let
    $H$ be the submatrix of the nonzero rows of $H'$.
\item{} [Inverse] Compute $S = (H^t)^{-1} M$ using the ``last big row'' trick \cite{cite Clement-Stein paper, which is now published}.
...
Let $A$ be an abelian variety of dimension $2$. After choosing a basis for
...
\end{lemma}
\begin{proof}
  If $x\in\Kbar$, this is standard Galois theory. \ws{How is this not super well known -- seems like we should just cite some book.} If $x\not\in
...
Let $T=\ZZ[T_1,T_2,\ldots]$ be the ring generated by the Hecke operators. A
result of Sturm yields an explicit integer $s$ such that $T$ is generated by
$T_1,\ldots,T_s$ as a $\ZZ$-module.

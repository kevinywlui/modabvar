...
\begin{algorithm}{Decompositon of jacobians}
    ...
    $A_i$ with finite intersection that sum to $J$. Let $L_1,\ldots, L_r$ be
    the divisors of $N$.
    \begin{enumerate}
        \item{} [New modular symbols]
            For each divisor $L_i$, compute the new modular symbol space
            $\mathbb{M}_2(L_i)_\mathrm{new}$ using William's book.
        \item{} [Factor the new modular symbols]
            ...
            \begin{enumerate}
                ...
                \item{} [Factor characteristic polynomial]
                    ...
                    polynomial of $T$ so that $f_j$'s are irreducible and
                    distinct.
                \item{} [Check?]
            ...
            \end{enumerate}
        \item{} [Image under degeneracy maps]
...
The goal here is to compute the rational torsion subgroup $J(\QQ)_\mathrm{tor}$. In
...
finite. A projection morphism is any surjective map $\pi:A\to B_1$ with $(\ker
\pi)^0 = B_2$. This morphism is not unique.

To construct one, let $\{\lambda_1,\ldots,\lambda_n\}$ be a basis for the
defining lattice of $A$.  For each $i$, write $\lambda_i=u_i+v_i$, where
$u_i\in V_1$ and $v_i\in V_2$. This is possible because $A=B_1+B_2$ though not
unique unless $B_1\cap B_2$ is trivial. An example of a projection map is the
map $\phi$ given on rational homology by the formula $\phi(\lambda_i)=u_i$.

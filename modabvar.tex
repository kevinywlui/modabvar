...
isogeny $F':A\to B'$ such that $\ker F'$ is exactly $K^0$.

\begin{algorithm}{Factoring out Component Group}
    We will compute a finite subgroup $G$ of $B$ such that the kernel of the
    composition $g:A\to B\to B/G$ has component group exactly $A[n]$ for some
    positive integer $n$. This implies that there exists a morphism $h$ such
    that $g \circ [n] = h$. This $h$ is the desired isogeny.
    \begin{enumerate}
        \item{} [Compute the saturation of $\Im F$]
            Let $\Lambda$ be the defining lattice of the image of
            $F$~\ref{sec:image} and $\Lambda_sat$~\ref{alg:saturate} the
            saturation of $\Lambda$.
        \item{} [Determine $n$]
            Solve the matrix equation $XM = B$, where $B$ is the 
    \end{enumerate}
\end{algorithm}

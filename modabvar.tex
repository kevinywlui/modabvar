...
can show reduction map $A_f(\Q)\to A_f(\FF_p)$ is injective on torsion points.
Hence, for any set, $P$, of odd primes not dividing $N$,
$\GCD(\{\#A_f(\FF_p):p\in P\})$ is a multiple of the $\Q$-rational torsion order. To
compute $\#A_f(\FF_p)$, we use the result in Agashe-Stein.


\subsubsection{The Rational Torsion Order}

When $A$ is not an elliptic curve, there is no known algorithm for explicitly
computing the rational torison order. There are 39 newforms of level up to 30.

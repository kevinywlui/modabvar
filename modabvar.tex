...
in $J$, $A$ be also be given explicit by the pair $(L, J)$. We refer to $L$ as
the \emph{defining lattice} of $A$.
...
\begin{algorithm}{Enumerate Modular Abelian Varieties}
    ...
    \begin{enumerate}
        ...
        \item{} [Factor into Hecke eigenspaces]
            ...
            simultaneous eigenspaces for $\T$. We can factor the new modular
            symbols space by computing the eigenspaces with respect to a
            sufficiently generic element of the Hecke algebra.
            \begin{enumerate}
...
To compute a $\ZZ$-basis for $\T$, we can naively
embed $\T$ in $\Mat_{d\times d}(\QQ)$. This is a bigger ambient ring than
necessary. Instead, there is a method to embed $\T$ inside of $\QQ^d$ as is
done here~\cite{MR2137350}.
...
Let $\Phi_f:\MM_k(N, \epsilon; \ZZ)\to \CC^d$ be the period map~\ref{} of

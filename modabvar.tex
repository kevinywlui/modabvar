...
\begin{algorithm}
    Let $V$ be a subspace of $H_1(J, \QQ)$. This algorithm will return True if
    $V$ corresponds to an abelian subvariety of $J$ and False otherwise. Let
    $N$ be the level of $J$ and $S$ be the set of newforms of level dividing
    $N$. Let $\T=\{T_1,\ldots\}$ be the Hecke algebra of $J$.
    \begin{enumerate}
        \item{} [Decompose into eigenspaces] 
            We first decompose $V$ into a direct sum $V=W_1\oplus \cdot \oplus
            ...
            \begin{enumerate}
                ...
                \item{} [Compare with newforms]
                    ...
                    k}$.
                \item{} [Single newform?]
                    If $S_{j, k}$ is empty for any $k=1,\ldots,n_j$, return
                    False. If $S_{j,k}$ is not singleton for any $k=1,\ldots,
                    n_j$, increment $j$ and return to (b). Otherwise, $S_{j,k}$
                    is singleton for all $k$ so let $S_{j,k}=\{f_k\}$ and
                    $W_k=U_{j,k}$.
            \end{enumerate}
        \item{} [Is isogenous to power?]
            ...
            \begin{enumerate}
                \item{} [Even dimension?] 
                    If the dimension of $W$ is not even, return False.
                \item{} 
            \end{enumerate}
    \end{enumerate}
\end{algorithm}
...
\begin{algorithm}
    Let $W$ be a Hecke eigenspace of $J$. This algorithm will return True if
    $V$ corresponds to an abelian subvariety of $J$ and False otherwise.
    \begin{enumerate}
        \item{} [Determine corresponding newform]
            Let $S$ be the set of newforms of level dividing $N$. If possible,
            find $f$ 
    \end{enumerate}
\end{algorithm}
